...
| :--- | :--- | :--- |
| **Движение** | Классические клавиши ZX Spectrum: **`Q`** (вверх), **`A`** (вниз), **`O`** (влево), **`P`** (вправо)<br>*(также поддерживаются стрелки `▲` `▼` `◀` `▶`)* | Экранный D-Pad |
...
├── tools/                   # Инструменты сборки и локальный сервер разработки
